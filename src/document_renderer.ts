...
/**
 * Convert Canvas to JPEG Blob. JPEG is the raster format embedded into the
 * generated PDF (DCTDecode), so we reuse the exact raster already rendered
 * on the high-resolution canvas instead of re-drawing.
 */
export function getCanvasJpegBlob(canvas: HTMLCanvasElement, quality: number = 0.92): Promise<Blob> {
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => {
      if (blob) resolve(blob);
      else reject(new Error('Canvas JPEG export failed'));
    }, 'image/jpeg', quality);
  });
}

/**
 * Build a minimal, dependency-free PDF (PDF 1.4, single A4-proportioned page)
 * that embeds a JPEG raster as a DCTDecode image XObject. The page media box
 * inherits the canvas aspect ratio so the document never distorts.
 *
 * Exported so the verification harness can assert on the generated bytes
 * without a browser canvas.
 */
export function buildPdfFromJpeg(imgBytes: Uint8Array, imgW: number, imgH: number): Blob {
  const pageW = 595.28;
  const pageH = Math.round((pageW * (imgH / imgW)) * 100) / 100;
  const pageWStr = pageW.toFixed(2);
  const pageHStr = pageH.toFixed(2);
  ...
  const obj3 = enc.encode(
    `3 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${pageWStr} ${pageHStr}] /Resources << /XObject << /Im0 4 0 R >> >> /Contents 5 0 R >>\nendobj\n`
  );
  ...
  const contentBytes = enc.encode(`q ${pageWStr} 0 0 ${pageHStr} 0 0 cm /Im0 Do Q`);
  ...
  const offsets: number[] = new Array(5);
  let cursor = 0;
  for (const part of parts) {
    part.offset = cursor;
    cursor += part.bytes.length;
    const head = new TextDecoder().decode(part.bytes.slice(0, 8));
    const objNum = parseInt(head, 10);
    if (objNum >= 1 && objNum <= 5) {
      offsets[objNum - 1] = part.offset;
    }
  }
  const xrefStart = cursor;

  const xrefBody = enc.encode(
    'xref\n0 6\n0000000000 65535 f \n' +
  ...
  const total = cursor + xrefBody.length;
  const out = new Uint8Array(total);
  for (const part of parts) {
    out.set(part.bytes, part.offset);
  }
  out.set(xrefBody, xrefStart);

  return new Blob([out], { type: 'application/pdf' });
}

/**
 * Export the already-rendered high-resolution canvas as a PDF document.
 * The only browser API used is canvas.toBlob (JPEG), so this also works
 * in the PWA offline context.
 */
export async function exportCanvasToPdf(canvas: HTMLCanvasElement): Promise<Blob> {
  const jpegBlob = await getCanvasJpegBlob(canvas, 0.92);
  const imgBytes = new Uint8Array(await jpegBlob.arrayBuffer());
  return buildPdfFromJpeg(imgBytes, canvas.width, canvas.height);
}
